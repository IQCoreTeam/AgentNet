// Skill sigil: from a skill's NAME we derive a deterministic "magic-circle" sigil SVG
// (concentric rings, radial ticks, an inscribed polygon/star, rune marks). Ported from
// surfaces/webview/src/market/skillSigil.ts; the panel bundle imports this module. Same name
// => same art, always. Not AI; a seeded mulberry32 PRNG, like the wallet avatar. The sigil
// colour is a fixed mono light-grey (the SD-card screen is near-black), so only the SHAPE
// varies per name.
function hash(s: string): number {let h=2166136261;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619);}return h>>>0;}
function rng(s: string){let a=hash(s);return function(){a|=0;a=(a+0x6d2b79f5)|0;let t=Math.imul(a^(a>>>15),1|a);t=(t+Math.imul(t^(t>>>7),61|t))^t;return((t^(t>>>14))>>>0)/4294967296;};}
export function skillSigilSvg(name: string): string {
  const col='hsl(228 5% 82%)';
  const r=rng((name||'')+'::sigil');
  ...
  const sides=3+Math.floor(r()*5); const rot=r()*Math.PI*2; const pr=12+r()*5; const pts: number[][]=[];
  ...
  if(sides>=5){const star: number[][]=[];for(let i=0;i<sides;i++)star.push(pts[(i*2)%sides]);const sp=star.map(function(p){return p[0].toFixed(1)+','+p[1].toFixed(1);}).join(' ');s+='<polygon points="'+sp+'" fill="none" stroke="'+col+'" stroke-width="0.6" opacity="0.22"/>';}
