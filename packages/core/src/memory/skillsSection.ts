 ...
 *  Pure filesystem, no RPC. Both runtimes install the same SKILL.md, so we read the
 *  appropriate directory based on which CLI is active. Returns [] when nothing is
 *  installed (or the dir is missing) — the caller then writes an empty/cleared block. */
