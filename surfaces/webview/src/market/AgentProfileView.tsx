...
// the WORK cards, the blog/comment bodies, the skill comment rows, and the feed
// (exported like CommentThreadList). `className` controls outer spacing.
...
export function BlogPostView({ post, wallet, onClose }: { post: BlogNote; wallet: string; onClose: () => void }) {
  ...
  function submitComment(f: NoteFields, parentId?: string) {
    ...
    send({ type: "postBlogComment", postId: post.id, agentWallet: wallet, text, gitLink: f.gitLink, parentId, feedBump: true });
  }
  return (
