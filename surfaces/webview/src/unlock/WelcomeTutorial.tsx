...
const CARDS: WelcomeCard[] = [
  {
    img: coderImg,
    position: "50% 30%",
    title: <>You are a<br />developer now</>,
    body: "Your phone is now a computer with a genius developer friend inside. You can build anything. Ask that friend to do anything for you.",
  ...
  },
  {
    ...
    body: "Every chat session is encrypted and picks up on any device: your PC too, even over remote access.",
    footnote: "All of this and more: just hit Full unlock in Settings later. Cloud backup and the PC install guide are covered there.",
  },
  {
    img: earnImg,
    position: "50% 40%",
    title: <>Make prompts,<br />earn money</>,
    ...
    advanceLabel: "OK",
    optOut: true,
    footnote: "Setup can wait. When you want to do something, drop by the market and set up then.",
  },
  {
    eyebrow: "GETTING_STARTED",
    title: <>Connect your<br />coding engine</>,
    body: "This is the only setup: connect claude or codex and you can start coding right away.",
    connect: true,
  },
];
      ...
      <section className="an-term-mono unlock-sheet relative z-10 flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden border border-b-0 border-[color:var(--an-line)] bg-[color:var(--an-bg-0)] text-[color:var(--an-fg)]">
        ...
        <div key={page} className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          <div className="mx-auto max-w-[320px] text-center">
            ...
            {card.connect ? (
              <>
                <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.14em] text-[color:var(--an-fg-mute)]">Connect</p>
                ...
                <button type="button" onClick={closeForNow} className="an-btn an-btn-outline mt-2 w-full">Not now</button>
              </>
            ) : (
              <>
                <button type="button" onClick={next} className="an-btn an-btn-green mt-3">{card.advanceLabel ?? "Next >"}</button>
                {card.optOut && (
