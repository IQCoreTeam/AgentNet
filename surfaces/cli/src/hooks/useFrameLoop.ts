...
export function useFrameLoop(frameCount: number, fps = 6, loop = true): number {
  const { animate, typing } = useDelight();
  const [frame, setFrame] = useState(0);

  useEffect(() => {
    // freeze (hold the current frame) while the user is typing — a repaint mid-IME
    // composition flickers the terminal's preedit text (Hangul etc.).
    if (!animate || typing || frameCount <= 1) return;
  ...
  }, [animate, typing, frameCount, fps, loop]);
