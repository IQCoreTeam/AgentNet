...
- **"터미널 축하창" 안에서 표현:** 화려함(파티클·모션·글로우)은 일반 파티 컨페티가 아니라 **터미널 셀러브레이션 창** 안에 담아 터미널 무드로 그린다. 예: 모노스페이스 카피, 그린/액센트 글로우, ASCII·그리드형 파티클, 커맨드 출력처럼 라인이 찍히는 reveal, 스캔라인/글리치 한 끗. "터미널이 축하해주는" 룩이지 범용 폭죽이 아님.
