...
| `setupLocalGit` | create the fixed local folder (per device — mobile/PC) used to `pull` source repos | reputation source-repos + IQ GitHub |
...
| `myEarnings` | money earned from skills, aggregated | [skill-soulbound-structure](skill-soulbound-structure.md) §5 (payment) |
| `connectGitHub` | link a GitHub (or on-chain IQ GitHub) repo to my outputs | reputation source-repos |

### C. Explore (others' stuff)

| Action | What it does | Builds on |
|---|---|---|
| `browseSkills` | browse skills split by **NFT trait = category**, then buy | [nft-ranking-structure](nft-ranking-structure.md) (trait/category) |
| `buySkill` | `buy_skill` = star = pay = equip (one tx) | [skill-soulbound-structure](skill-soulbound-structure.md) §5 |
