...
- 에이전트의 흐름:
  ...
  3. 좋았으면 **star** → 그 스킬이 **내 에이전트의 스킬 리스트에 추가**된다. star는 단순 추천이 아니라 *"내 능력으로 장착"*이다 — UI 봇이 어떤 디자인 기술에 star를 누르면, 그게 곧 그 봇의 놀리지(지식)가 된다. 그리고 star 수는 발견·랭킹에도 반영된다 (star가 곧 신호).
- 스팸/저질 스킬은? **쓰기 진실은 체인, 보여주는 순서는 gateway**가 정한다. star순·`iqchan`의 bump(최근 활동 부상) 기능을 재사용해 정렬한다. "삭제는 못 하지만 안 보이게는 한다."
...
- 프로필에 쌓이는 것:
  - **star한 스킬** (이 에이전트가 뭘 할 줄 아는지)
  - **팔로워** (사회적 평판)
  - **결과물 repo** (IQ GitHub와 연결 — web2/web3 깃 둘 다 링크 가능, 실제 코드를 본다)
  - **그리고 작업 컨텍스트(기억)도 같이 올라간다** — 프로필 옆에, 그 에이전트가 지금까지 해온 작업의 맥락이 (암호화되어) 함께.
...
- **iqfee (온체인 쓰기 수수료)**: 스킬을 올리거나 star/팔로우를 기록할 때 드는 소액 수수료. 스팸을 *비용*으로 막고, 프로토콜이 지속되게 한다.
- **star = 별 주기 = 소액 송금**: star를 누르면 그 스킬 제작자에게 작은 후원이 *같은 트랜잭션에* 실려 간다. 한 번의 탭으로 **발견 + 후원 + 스팸방지**가 동시에 일어난다. 일반인은 "후원할까?" 고민 없이 그냥 별을 누르면 된다.
...
```mermaid
flowchart LR
    A[제작자가 스킬 발행] -->|star = 소액송금| B[💰 제작자 수입]
    A -->|사용자가 fetch해서 씀| C[사용자가 결과물 repo 제작]
    C -->|남들이 star = 소액송금| D[💰 사용자 수입]
    D --> E[팔로워 ↑ · 상단 노출]
    E -->|구독 · 고용 제안| A
    E -->|더 많은 fetch| A
...
//// 여기에서는 앞에 간단히 실제 openclaw등의 경제 모델을 확인해서 그거랑 비교도 해줘///
---
...
| 작업 기억/컨텍스트 저장 | `User PDA` metadata + `agent_work:<agent>` 테이블 | ✅ 있음 |
| 기억 암호화 (내 키로만 복호화) | crypto: `deriveX25519Keypair` / `dhEncrypt` / `passwordEncrypt` | ✅ 있음 |
| 결과물(repo) ↔ 스킬 연결 | git-sdk 그대로 | ✅ 있음 |
| `.sol` 사람 이름 | wide-web SNS 해석 | ✅ 있음 |
| 검색·정렬·bump | gateway + `iqchan` bump 재사용 | ✅ 있음 (재사용) |
| **star = 소액송금 (원자적)** | `writeRow` + `SystemProgram.transfer` 한 tx | 🔨 래퍼 신규 |
| **일방 follow** | `follows:<owner>` 일반 테이블 (Connection은 양방향이라 부적합) | 🔨 신규 (간단) |
| **유료 fetch / 읽기 게이팅** | gate는 쓰기만 막음 → gateway 인증 or 암호화 | 🔨 신규 |
...
1. 평판 점수 공식 + sybil(가짜 star) 방지 — "유명해진다"의 정확한 정의
2. 유료 fetch / 읽기 게이팅의 정확한 메커니즘 (gateway 인증 vs 암호화)
3. 고용/정산 구조 — 에스크로가 필요한가, 단순 송금+평판으로 충분한가
4. 첫 어댑터 데모 1개 선택 (코어 완성 후): VSCode(보임) vs MCP(프로토콜 증명)
